				...
				<div class="col large-7 small-12">
					...
					foreach ($package_product_ids as $opt_i => $prod) {
						...
						<?php
							...
							if ($prod['type'] == 'free') {
								...
								$r_price_total = $product_regular_price * $i_total_qty;
								...
								$r_price_total = $product_regular_price * $prod['qty'];
							...
							if ($prod['type'] == 'free' || $prod['type'] == 'off') { ?>
								...
								<div class="col-4 item-selection col-hover-focus mwc_item_div mwc_item_div_<?php echo ($p_id) ?> op_c_package_option <?= (self::$package_default_id == $prod['bun_id']) ? 'mwc_selected_default_opt' : '' ?>" data-type="<?php echo ($prod['type']) ?>" data-bundle_id="<?php echo (trim($prod['bun_id'])) ?>" data-cupon="<?= round($i_cupon, 0) ?>">
								<?php
							} else { ?>

                                <div class="col-4 item-selection col-hover-focus mwc_item_div mwc_item_div_<?php echo ($prod['bun_id']) ?> op_c_package_option <?= (self::$package_default_id == $prod['bun_id']) ? 'mwc_selected_default_opt' : '' ?>" data-type="<?php echo ($prod['type']) ?>" data-bundle_id="<?php echo (trim($prod['bun_id'])) ?>" data-cupon="<?= round($i_cupon, 0) ?>">
                            ...
                            <?php } ?>
									...
									<div class="col-inner">
										...
										<div class="op_c_package_content">
											...
											<!-- discount percent -->
											<?php if (0 != $i_cupon) : ?>
												<h4 class="mwc-discount-text mt-2 mb-0"><?php echo (__('Save', 'mwc')) ?> : <?php echo (round($i_cupon, 0)) ?>%</h4>
											<?php endif; ?>
											...
											<?php if ($prod['type'] == 'bun') { // bundle option ?>
												<h3 class="mwc-sub-price mt-4 mb-0">
													<strong><?= __('Bundle price', 'mwc') ?>:</strong>
												...
												?>
													<h3 class="mwc-sub-price mt-4 mb-0">
														<strong><?php echo wc_price($i_price); ?></strong>
														<span class="pi-price-each-txt"> / <?php echo __('each', 'mwc'); ?></span>
													</h3>

													<h4 class="mwc-total-price mt-2">
														<?php echo wc_price($r_price_total); ?> <?php echo wc_price($i_price_total); ?>
													</h4>
													...
													<input type="hidden" class="mwc_bundle_price_sale_hidden" data-label="<?= __('Sale Price', 'mwc') ?>" value="<?= $product_sale_price ?>">
					...
					foreach ($package_product_ids as $opt_i => $prod) {
						...
						<?php
							...
							<div class="mwc_product_variations mwc_product_variations_<?php echo (trim($prod['bun_id'])) ?> info_products_checkout <?= (($prod['type'] == 'free' || $prod['type'] == 'off') && $product->is_type('variable')) ? 'is_variable' : '' ?>" data-bundle_id="<?php echo (trim($prod['bun_id'])) ?>">
